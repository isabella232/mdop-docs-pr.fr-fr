...
---
# Notes de publication pour Microsoft User Experience Virtualization (UE-V)2.1SP1


Pour effectuer une recherche dans les notes de publication de Microsoft User interface 2,1 SP1, appuyez sur Ctrl + F.
...
Cette section contient des notes de publication relatives à l’utilisation de User Virtualization 2,1 SP1.
...
UE-V inclut un programme d’installation localisé pour les agents UE-V et UE-V. Ils sont toujours disponibles, mais l’interface utilisateur est réduite et l’affichage du MSI uniquement en anglais. Malgré le fichier en anglais, le programme d’installation installe toutes les langues prises en charge lors de l’installation.
...
Solution de contournement: lorsque vous migrez d’UE-V 1 vers UE-V 2 et qu’il est probable que des ordinateurs exécutent la version précédente de l’agent, vous devez créer un catalogue UE-V 2. x distinct pour prendre en charge l’agent et les modèles UE-V 2. x.

### Retard de la fermeture de session UE-V

Occasionnellement lors de la fermeture de session, UE-V met trop de temps à synchroniser les paramètres. En règle générale, il s’agit d’un réseau à forte latence ou d’une utilisation incorrecte du système de fichiers Distrubuted.
Pour la prise en charge du système de fichiers DFS, consultez [l’instruction de support de Microsoft concernant les données de profil utilisateur répliquées](https://support.microsoft.com/kb/2533009) pour obtenir des informations supplémentaires.

Solution de contournement: à partir de HF03, une nouvelle clé de registre a été ajoutée la clé de Registre suivante fournit un mécanisme selon lequel le délai de déconnexion maximal peut être spécifié \\Software\\Microsoft\\UEV\\Agent\\Configuration\\LogOffWaitInterval

Pour plus d’informations [, voir paramètres de Registre UE-V](https://support.microsoft.com/kb/2770042) .

## HotFix et Articles de la base de connaissances pour UE-V 2,1 SP1


Cette section contient des correctifs et des Articles de la base de connaissances pour UE-V 2,1 SP1.
